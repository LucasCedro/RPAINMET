...
# Cria um pool de trabalhadores com até 4 threads
with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
    # Mapeia a função insert_data_from_csv para cada arquivo em file_names
    futures = {executor.submit(insert_data_from_csv, os.path.join(directory_path, file_name), engine): file_name for file_name in file_names}

    # À medida que os trabalhos são concluídos, imprima o tempo que levou para cada um
    for future in concurrent.futures.as_completed(futures):
        file_name = futures[future]
        try:
            future.result()
        except Exception as exc:
            print(f'{file_name} gerou uma exceção: {exc}')
        else:
            print(f'{file_name} concluído com sucesso')

end_time_1 = time.time()

print(f"Tempo total: {round(end_time_1 - start_time_1, 1)} segundos")